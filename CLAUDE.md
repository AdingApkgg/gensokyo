...
- 认证（邮箱验证 / Google / 找回密码，M6，已完成）约定：
  ...
  - **`apps/api` 的 `test` 与 `e2e` 脚本硬前缀 `MAIL_TRANSPORT=console`，不要删。** 实测 shell 环境变量优先于 `--env-file`（`bun run` 与 `bun test` 都是），所以这个前缀能压住本地 `.env` 里的任何配置。没有它的话，本地把 `MAIL_TRANSPORT` 配成 resend/smtp 之后：① 每跑一次测试就往真实邮箱发一批信；② **e2e 直接失败**——它靠读 stdout 取验证码，而那两个通道不打 stdout。「测试绝不发真信」必须是脚本层面的保证，不能靠人记得改 `.env`
