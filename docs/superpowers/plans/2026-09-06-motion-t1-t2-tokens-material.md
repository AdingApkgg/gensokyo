...
**spec 有、本期没做、也不该悄悄消失的两条**（最终审查指出它们既未实现也未列入本节）：

- spec §4.2 的「portal 弹层消散 240ms」——Task 2 只把 `duration-100` 换成 `duration-sumi`，进出都是对称的 180ms
- spec §4.2 的「出场降一档（÷√2）」——同上，本期没有任何地方区分进场/出场时长

两条都属于「时长精修」，需要在真实浏览器里反复看才调得准，而本期的验证环境（隐藏面板、动画被节流）做不到。留给 T3 或一次专门的观感调优。
