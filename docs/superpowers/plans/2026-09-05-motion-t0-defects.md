...
**这一整块必须不带任何 `@layer`，插在 `apps/web/app/app.css` 文件末尾**（`@layer utilities { … }` 块之后）。**不能放进 `@layer base`**：设 `--tw-enter-*`/`--tw-exit-*` 非零值的工具类在 `@layer utilities`，Tailwind v4 层序 theme→base→components→utilities 晚层恒胜，放 `base` 会被 `utilities` 完全盖过而失效——这是最终审查在生产构建产物上实测抓出的错误，不是靠读源码能看出来的，因为源码层面两条规则都「看起来」在起作用。也**不能放进 `@layer utilities`**：我们的 `*` 特异性 0,0,0 仍输给工具类选择器 0,1,0。未分层的常规声明胜过任何分层声明，这是唯一有效的位置：

```css
/* 减弱动效：只掐掉位移与缩放，保留淡入淡出与颜色过渡。
   **整块必须不带 @layer**：设 --tw-enter-* 与 --tw-exit-* 的工具类在 @layer utilities，
   Tailwind v4 层序 theme→base→components→utilities 晚层恒胜，放进 @layer base 会完全失效；
   放进 @layer utilities 也不行——我们的 `*` 特异性 0,0,0 仍输给工具类选择器 0,1,0。
   未分层的常规声明胜过任何分层声明。

   三条写法上的选择也都是刻意的：
   1) 选择器必须是 `*, *::before, *::after`——仓库弹层的类名 token 是 `data-open:animate-in`，
      `:where(.animate-in)` 一个元素都选不中。
   2) 不能用 `animation: none`——Radix 靠 animationend 卸载弹层，掐掉动画会让弹层卡住不消失。
      归零变量保留淡入淡出、只掐位移与缩放。
   3) 不能用 `transition-duration: .01ms !important`——会连颜色过渡一起杀，与后续
      MotionConfig reducedMotion="user"（只关 transform、保留 opacity）的语义打架。 */
...
`bun run check-css-layers` 解析构建产物、断言这条归零规则不在任何 `@layer` 内，把这条约定钉成门禁。
