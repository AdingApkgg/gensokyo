...
> **状态：2026-09-05 搁置。** 不是设计有误，是**时机不对**——决定做全量镜像时没有先看站的真实规模。
>
> 当时生产站：3 个账号、**0 个社团**、7 条楼层（全是播的引导帖）、103 条种子资源。
> 为一个 0 社团的站镜像 20,779 个社团、233,511 条实体，投入与收益不成比例。
>
> **本设计里未解的洞**：中文层的价值全部来自译名，而译名供给没有来源。
> 「staff 导入」写在第九节，但站长手上并没有 23 万条中文译名；THBWiki 有，
> 而抓它既是另一个项目，也与「不正面做百科」的决定冲突。按本设计上线，
> 第一天的 /chronicle 是一个更慢的 TouhouDB 日文镜像，配一个中文搜不到东西的搜索框。
> 23 万个只有日文原名加一条外链的页面也不是 SEO 资产，是薄内容。
>
> **代码去向**：T1 契约层与 T2 schema 已实现并测试通过，保存在分支 `m5-chronicle`
> （提交 `77edbf2`、`d96d884`）。main 已退回，开发库的迁移 0007 已回滚，生产从未应用。
>
> **复活条件（两个都要成立）**：站上有人在搜索；译名有稳定来源。
> 后者的自然来源是投稿流程——投稿者绑定社团时顺手填的中文名，一条条攒起来。
> 那时候中文层是从真实使用里长出来的，而不是从一次 0% 覆盖率的批量导入里硬灌的。
>
> **更小的替代方案**：香霖堂投稿表单里按中文搜社团、现调 TouhouDB API 带出原名与封面。
> 不建表、不同步、不触发「能不能转载别人数据库」的许可问题。约一天工作量，今天就有用户。
> 第三节的实测事实全部仍然有效，复活时不必重测。
